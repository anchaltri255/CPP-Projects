...
int main() {
	...
	if(!mysql_reat_connect(conn,HOST,USER,PW,DB,3306,NULL,0)){
		cout<<"Error: "<<mysql_error(conn)<<endl;
	}
	else{
	...
	string insert3 = "INSERT INTO Airline (Fnumber, Departure,Destination, Seat) VALUES('"+f3.getFnum()+"','"+f3.getDep()+"','"+f3.getDes()+"','"+strS1+"')";
	...
	cout<<"*************************************"<<endl;
	cout<<"1. Reseve A Seat In Flight: "<<endl;
	cout<<"2. Exit: "<<endl;
	...
	if(val==1){
		...
		if(total>0){
			...
			if(mysql_query(conn,update.c_str())){
				...
				cout<<"Seat is Reserve Successfully in: "<<flight<<endl;
			}
			if(total==0){
